...
public class CompatModelBuilder {
    ...
    public CompatModelBuilder toHumanoid() {
        ...
        for (String name : HUMANOID_PARTS) {
            // hat は head の子パーツ
            if (name.equals(HAT)) continue;
            if (!newRoot.hasChild(name))
                newRoot.addChild(name);
        }

        // 元のモデルを書き換えないよう head は複製してから hat を補う
        CompatModelPart head = newRoot.getChild(HEAD);
        CompatModelPart newHead = new CompatModelPart(head.cubes, head.pose);
        newHead.children.putAll(head.children);
        if (!newHead.hasChild(HAT)) {
            // BlockBench のモデルでルート直下に hat がある場合は head の下へ移す
            CompatModelPart rootHat = newRoot.children.remove(HAT);
            if (rootHat != null) newHead.children.put(HAT, rootHat);
            else newHead.addChild(HAT);
        }
        newRoot.children.put(HEAD, newHead);
    ...
    public LayerDefinition toLayerDefinition() {
        return LayerDefinition.create(toMesh(), textureWidth, textureHeight);
    }

    public ModelPart bake() {
        return toLayerDefinition().bakeRoot();
    }
