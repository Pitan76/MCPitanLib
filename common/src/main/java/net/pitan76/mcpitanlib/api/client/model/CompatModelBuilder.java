...
public class CompatModelBuilder {
    ...
    /**
     * 指定したパーツをこのバージョンのモデル形式 (ModelPart) に変換する
     */
    public ModelPart bake(String partName) {
        CompatModelPart part = root.getChild(partName);
        if (part == null) return new ModelPart(textureWidth, textureHeight, 0, 0);
        return part.bake(textureWidth, textureHeight);
    }

    /**
     * 人型モデルとして変換する ({@link #toHumanoid()} したものに対して使う)
     * このバージョンではバニラ側が部位ごとにパーツの表示を上書きするため、
     * slot を指定した場合はその部位以外のパーツを空にする
     */
    public BipedEntityModel<LivingEntity> bakeHumanoid(EquipmentSlot slot) {
        BipedEntityModel<LivingEntity> model = new BipedEntityModel<>(0.0F);
        boolean all = slot == null;
        boolean head = all || slot == EquipmentSlot.HEAD;
        boolean chest = all || slot == EquipmentSlot.CHEST;
        boolean legs = all || slot == EquipmentSlot.LEGS || slot == EquipmentSlot.FEET;

        model.head = head ? bake(HEAD) : empty();
        model.hat = head ? bake(HAT) : empty();
        model.body = chest ? bake(BODY) : empty();
        model.rightArm = chest ? bake(RIGHT_ARM) : empty();
        model.leftArm = chest ? bake(LEFT_ARM) : empty();
        model.rightLeg = legs ? bake(RIGHT_LEG) : empty();
        model.leftLeg = legs ? bake(LEFT_LEG) : empty();
        return model;
    }

    private ModelPart empty() {
        return new ModelPart(textureWidth, textureHeight, 0, 0);
    }
    ...
    public EntityModelLayerContext build() {
        return new EntityModelLayerContext(textureWidth, textureHeight);
    }
}
