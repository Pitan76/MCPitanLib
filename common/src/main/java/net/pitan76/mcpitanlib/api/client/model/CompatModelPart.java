...
public class CompatModelPart {
    ...
    /**
     * このバージョンのモデル形式 (ModelPart) に変換する
     */
    public ModelPart bake(int textureWidth, int textureHeight) {
        ModelPart part = new ModelPart(textureWidth, textureHeight, 0, 0);
        pose.applyTo(part);
        cubes.addTo(part);
        children.values().forEach(child -> part.addChild(child.bake(textureWidth, textureHeight)));
        return part;
    }
}
