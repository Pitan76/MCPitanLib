...
public class CompatModelPart {
    ...
    protected void addChildrenTo(ModelPartData parent) {
        children.forEach((name, child) -> child.addTo(parent, name));
    }

    protected void addTo(ModelPartData parent, String name) {
        ModelPartData part = parent.addChild(name, cubes.toRaw(), pose.toRaw());
        addChildrenTo(part);
    }
}
