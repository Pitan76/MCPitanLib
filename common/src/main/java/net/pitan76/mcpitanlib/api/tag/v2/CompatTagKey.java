...
public class CompatTagKey<T> extends TagKey<T> {
    @Deprecated
    public CompatTagKey(net.minecraft.registry.tag.TagKey<T> tagKey) {
        super(tagKey);
    }

    @Deprecated
    public static <T> CompatTagKey<T> of(net.minecraft.registry.tag.TagKey<T> tagKey) {
        return new CompatTagKey<>(tagKey);
    }

    public static <T> CompatTagKey<T> of(CompatTagKeyType<T> type, CompatIdentifier identifier) {
        return of(net.minecraft.registry.tag.TagKey.of(type.getRegistryKey(), identifier.toMinecraft()));
    }
