...
public class RecipeUtil {
    ...
    public static <I extends Inventory, T extends Recipe<I>> CompatRecipeEntry<T> getFirstMatch(RecipeManager recipeManager, CompatRecipeType<T> type, CompatRecipeInput<I> input, World world) {
    ...
    public static <I extends Inventory, T extends Recipe<I>> CompatRecipeEntry<T> getFirstMatch(RecipeManager recipeManager, CompatRecipeType<T> type, CompatRecipeInput<I> input, World world, CompatIdentifier identifier) {
        Optional<Pair<Identifier, RecipeEntry<T>>> recipe = recipeManager.getFirstMatch(type.getType(), input.getInput(), world, identifier.toMinecraft());
        return recipe.map((r) -> new CompatRecipeEntry<>(r.getSecond())).orElseGet(() -> new CompatRecipeEntry<>(null));
    }

    public static <I extends Inventory, T extends Recipe<I>> CompatRecipeEntry<T> getFirstMatch(World world, CompatRecipeType<T> type, CompatRecipeInput<I> input) {
        return getFirstMatch(getRecipeManager(world), type, input, world);
    }

    public static <I extends Inventory, T extends Recipe<I>> CompatRecipeEntry<T> getFirstMatch(World world, CompatRecipeType<T> type, CompatRecipeInput<I> input, CompatIdentifier identifier) {
    ...
    public static ItemStack getOutput(Recipe<?> recipe, CompatRegistryLookup registryLookup) {
        if (registryLookup.getRegistryLookup() instanceof DynamicRegistryManager) {
            return recipe.getResult((DynamicRegistryManager) registryLookup.getRegistryLookup());
        }
    ...
    public static  <I extends Inventory, T extends Recipe<I>> ItemStackList getRemainder(CompatRecipeEntry<T> recipeEntry, CompatRecipeInput<I> input) {
