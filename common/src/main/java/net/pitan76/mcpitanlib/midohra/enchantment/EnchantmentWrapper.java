 ...
 * エンチャントへの参照。
 * <p>
 * このバージョンではエンチャントは静的レジストリにあるが、
 * 1.21以降はデータパックレジストリへ移りワールドを読み込むまで実体が存在しない。
 * 両方で同じ書き方ができるよう、IDだけを保持して必要なときに引く形にしている。
 */
public class EnchantmentWrapper {

    private final CompatIdentifier id;

    protected EnchantmentWrapper(CompatIdentifier id) {
        this.id = id;
    }
    ...
    public Optional<Enchantment> getEntry(World world) {
        return Registry.ENCHANTMENT.getOrEmpty(id.toMinecraft());
    }
    ...
    public boolean equals(Object obj) {
        ...
        return ((EnchantmentWrapper) obj).getId().equals(getId());
    }
